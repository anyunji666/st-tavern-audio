...
基于你提供的 st-immersive-sound 项目大幅精简重构而来，只保留你明确要的功能：
...
1. **豆包接口鉴权字段**：我按你原项目 `tts.js` 里的
   `X-Api-App-Key` / `X-Api-Access-Key` / `X-Api-Resource-Id` 三个请求头实现，和原来完全一致。如果你后来换过接口版本，请对照实际豆包文档核对。
2. **NPC音色映射的作用域**：目前是全局的（`extension_settings` 级别，不分角色卡/不分聊天）。如果你希望不同角色卡各自维护一套NPC映射（避免"老板娘"在不同故事里被绑死同一个音色），告诉我，我可以加一层按 `characterId` 分区。
3. **LLM输出格式**：约定LLM必须返回一段JSON（prompt模板里有完整格式说明）。如果你常用的模型不太听话、经常输出不规范JSON，可以在"LLM解析"页调整 prompt 模板，或者告诉我你常用哪个模型，我再针对性调一下解析容错。
4. 我没有对接真实的豆包/MiMo/Edge-TTS接口做实际请求测试，只做了语法和调用链路检查，建议你先用一条短文本跑一次全链路，把日志（浏览器控制台）发我，方便对第一次联调做修正。
5. **Edge-TTS 的稳定性说明**：它是纯浏览器端直连微软内部服务的 WebSocket 实现，无需 API Key，但也没有官方保障——微软近年加了一个基于时间戳的反爬校验（Sec-MS-GEC），代码里已经按社区通用算法实现，正常情况下能用；如果之后微软改协议或加更严格的校验导致连不上，这个供应商可能会失效，到时候需要再调整。
